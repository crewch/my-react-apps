...
function App() {
	...
	const sortedAndSearchedPosts = usePosts(arrPosts, filter.sort, filter.string)
	...
	return (
		<div style={{ backgroundColor: '#f0f0f0' }}>
			...
			<div style={{ display: 'flex' }}>
				<button onClick={() => setModalState(true)} className={style.createBnt}>
					Создать пост
				</button>
				<PostFilter filter={filter} setFilter={setFilter} />
				<div>
					<button className={style.createBnt} onClick={() => setLimit(10)}>
						лимит 10
					</button>
					<button className={style.createBnt} onClick={() => setLimit(20)}>
						лимит 20
					</button>
				</div>
			</div>
			{isLoading ? (
				...
				<Posts deletePost={deletePost} arrPosts={sortedAndSearchedPosts} />
			)}
